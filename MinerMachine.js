...
const { CreepMachine } = require("./CreepMachine");


class MinerMachine extends CreepMachine{
    ...
    static do_schedule(creep) {
        ...
        if (creep.obj.store.getUsedCapacity(RESOURCE_ENERGY) == 0) {
            target_list = creep.obj.room.find(FIND_MY_STRUCTURES, {filter: (s) => {return (s.structureType == STRUCTURE_CONTAINER && s.store.getFreeCapacity(RESOURCE_ENERGY) > 0)}})
